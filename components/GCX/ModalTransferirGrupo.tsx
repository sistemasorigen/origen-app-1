...
import { ArrowLeftRight, Search, AlertTriangle, ChevronLeft, Loader2, Lock, RotateCcw } from 'lucide-react';
...
interface ModalTransferirGrupoProps {
    isOpen: boolean;
    onClose: () => void;
    group: Group;
    currentUser: User;
            ...
            {steps.map((label, i) => {
                ...
                return (
                    <React.Fragment key={n}>
                        ...
                        >
                            <div
                                className={`w-7 h-7 flex items-center justify-center border-2 font-black text-xs transition-all duration-200
...
// ── Main Component ─────────────────────────────────────────
const ModalTransferirGrupo: React.FC<ModalTransferirGrupoProps> = ({
    isOpen,
    onClose,
    group,
    currentUser,
    ...
    // Versión para forzar re-búsqueda sin cambiar el term
    ...
    // Auto-focus confirm input when reaching step 3
    useEffect(() => {
        if (step === 3) {
    ...
    const handleConfirm = async () => {
        if (!selectedUser || confirmText !== 'Transferir') return;
        setLoading(true);
        setError(null);
            ...
            <div className="px-1 pb-1">
                    ...
                    <div>
                        ...
                        <div className="mb-3">
                            ...
                            <div className="relative">
                                ...
                                {searching ? (
                                    <Loader2
                                        className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400 animate-spin"
                                        aria-hidden="true"
                                    />
                                ...
                                <div className="border-2 border-black overflow-hidden max-h-60 overflow-y-auto">
                                    {searchResults.map((user, i) => (
                                        <div
                                            ...
                                            className={`flex items-center justify-between p-3 min-h-[52px] transition-all duration-150
                                                ${i < searchResults.length - 1 ? 'border-b border-neutral-100 dark:border-neutral-800' : ''}
                                                ${user.isHost
                                                    ? 'hover:bg-purple-50 dark:hover:bg-purple-950/30 cursor-pointer focus:outline-none focus:bg-purple-50 dark:focus:bg-purple-950/30'
                                                    : 'opacity-50 cursor-not-allowed bg-neutral-50 dark:bg-neutral-900/50 select-none'
                                                }`}
                                        >
                            ...
                            {searchTerm.trim().length >= 2 && !searching && searchResults.length === 0 && (
                                <div className="border-2 border-dashed border-neutral-200 dark:border-neutral-700 p-6 text-center">
                                    ...
                                    <p className="text-xs text-neutral-400 mt-1">
                                        Probá con nombre completo o email exacto
                                    </p>
                                </div>
                            )}
                            ...
                            {searchTerm.trim().length < 2 && (
                                <div className="p-4 text-center">
                                    ...
                                    <p className="text-xs text-neutral-400">
                                        Escribí nombre o email del nuevo anfitrión
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div>
                        ...
                        <div className="border-2 border-black dark:border-white p-4 mb-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.3)]">
                            <div className="flex items-start justify-between gap-2 mb-3">
                                ...
                                <span className="text-[9px] font-black uppercase px-2 py-1 bg-purple-600 text-white shrink-0">
                            ...
                            <div className="space-y-2">
                                ...
                                {selectedUser.phone && (
                                    <div>
                                        ...
                                        <p className="font-bold text-sm text-black dark:text-white">{selectedUser.phone}</p>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                {step === 3 && selectedUser && (
                    <div>
                        ...
                        {/* Confirm input with accessible label */}
                        <div className="mb-4">
                            <label
                                htmlFor="confirm-transfer"
                                className="block text-xs font-bold text-black dark:text-white mb-2"
                            >
                            ...
                            <input
                                ...
                                autoComplete="off"
                                className={`w-full h-11 px-3 border-2 font-bold text-black dark:text-white dark:bg-black bg-white focus:outline-none transition-all duration-150
                        ...
                        {error && (
                            <div
                                id="confirm-error"
                                role="alert"
                                className="flex items-start gap-2 text-xs font-bold text-red-600 dark:text-red-400 mb-4 border-2 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 px-3 py-2"
                            >
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className={`flex-[2] min-h-[44px] font-black uppercase tracking-widest text-xs border-2 transition-all duration-150 flex items-center justify-center gap-2
                                    ${canConfirm
                                        ? 'bg-black dark:bg-white border-black dark:border-white text-white dark:text-black hover:bg-white hover:text-black dark:hover:bg-black dark:hover:text-white hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] cursor-pointer'
                                        : 'bg-neutral-200 dark:bg-neutral-800 border-neutral-300 dark:border-neutral-700 text-neutral-400 cursor-not-allowed'
                                    }`}
...
export default ModalTransferirGrupo;
