...
import { Save, Crown, Search, ChevronDown, Calendar, Loader2 } from 'lucide-react';
...
const CrearGrupoAdminContent: React.FC = () => {
    ...
    const [form, setForm] = useState({
        ...
        location: '', description: '', maxCapacity: 12,
        imageUrl: '', coHostFirstName: '', coHostLastName: '',
        minAge: 0, maxAge: 100, targetGender: 'Mixto',
        tags: [] as string[], startDate: '', endDate: '',
    ...
    useEffect(() => {
        ...
        return () => clearTimeout(timer);
    }, [coHostSearchTerm, selectedHostId]);
    ...
    const toggleTag = (tagName: string) => {
        setForm(prev => {
            ...
            return currentTags.includes(tagName)
                ? { ...prev, tags: currentTags.filter(t => t !== tagName) }
                : { ...prev, tags: [...currentTags, tagName] };
        });
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!hasSelectedHost && !hasManualHost) {
            showToast('Debes asignar un Anfitrión (buscando o ingresando manualmente).', 'error');
            return;
        }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                    <div className="space-y-1">
                        ...
                        <input type="text" name="name" value={form.name} onChange={handleChange} className="w-full h-12 px-3 border-2 border-black rounded-none outline-none font-bold text-lg md:text-xl focus:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all" required />
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                <select name="categoryId" value={form.categoryId} onChange={handleChange} className="w-full h-12 px-3 border-2 border-black rounded-none outline-none font-bold bg-white appearance-none">
                ...
                <div className="space-y-1">
                    ...
                    <textarea name="description" value={form.description} onChange={handleChange} rows={3} className="w-full p-3 border-2 border-black rounded-none outline-none font-medium resize-none" />
                </div>
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {availableTags.map(tag => {
                                ...
                                return (
                                    <button key={tag.id} type="button" onClick={() => toggleTag(tag.id)} className={`px-3 py-1 text-[10px] font-black uppercase border-2 transition-all ${isSelected ? 'bg-black text-white border-black' : 'bg-white text-gray-500 border-gray-300'}`}>
                                        {tag.name}
                                    </button>
                                );
                            })}
                ...
                <div className="flex md:justify-end gap-3">
                    ...
                    >
                        {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Save className="w-5 h-5" /> Crear Grupo</>}
                    </button>
                </div>
            </form>
        </div>
